...
export default class InteropServiceHelper {
	...
	public static async export(_dispatch: Function, module: ExportModule, options: ExportNoteOptions = null) {
		...
		_dispatch({
			type: 'INTEROP_EXPORT_EXEC',
			path: path,
			feedback: _('Exporting to "%s" as "%s" format. Please wait...', path, module.format),
		});
		...
		try {
			...
			_dispatch({
				type: 'INTEROP_EXPORT_COMPLETE',
				path: path,
				feedback: _('Successfully exported to %s.', path),
			});
		} catch (error) {
			console.error(error);
			...
			_dispatch({
				type: 'INTEROP_EXPORT_COMPLETE',
				path: path,
				feedback: _('Could not export notes: %s.', error.message),
			});
		}
	}
